...
const setup = require("../../../../../Triple/src/configs/sunucuayar.json");
...
module.exports = async (message) => {
    ...
    if ((message.mentions.roles.size + message.mentions.users.size + message.mentions.channels.size) >= 7) return sendChat(message, "Birden çok kişiyi etiketlemekten vazgeçiniz")
    ...
    if(message.content && message.content.length && message.content.length >= 350)  return sendChat(message, "Bro bak bu mesaj çok uzun tamam mı romanmı yazdın lütfen uzun mesaj göndermemeye özen gösteriniz")
