...
module.exports = async (oldMember, newMember) => {
  if (newMember.roles.cache.size > oldMember.roles.cache.size) {
    ...
    const row = new ActionRowBuilder().addComponents(
      ...
      .setDescription(`
      ${entry.executor} üyesi cyr0nix'dan izinsiz yönetici rolü verdi ve üyeden rolü alıp, rolü veren kişiyi banladım.
      ─────────────────────
      ...
      let oziGuardLog = await newMember.guild.channels.cache.find(x => x.name == "guard_log").send({ embeds: [ozi], components: [row] });
