...
module.exports = async (oldMember, newMember) => {
    ...
    const data = await isimler.findOne({ guildID: allah.GuildID, userID: newMember.user.id });
    if(conf.sahipRolu.some(x => newMember.roles.cache.has(x))) return;
    let user = newMember;
    let kanal = client.channels.cache.find(c => c.name === "boost_log")
    if (!kanal) return;

    const tagModedata = await regstats.findOne({ guildID: allah.GuildID })
    if (tagModedata && tagModedata.tagMode === false) {
    if(!user.user.displayName.includes(conf.tag) && !user.roles.cache.has(conf.vipRole)) return;
    ...
    if(user && user.manageable) await user.setNickname(`${user.user.displayName.includes(conf.tag) ? conf.tag : (conf.ikinciTag ? conf.ikinciTag : (conf.tag || ""))} İsim | Yaş`)
    ...
    if(user && user.manageable) await user.setNickname(`${user.user.displayName.includes(conf.tag) ? conf.tag : (conf.ikinciTag ? conf.ikinciTag : (conf.tag || ""))} ${isim}`)
    }
    }
    };
};
