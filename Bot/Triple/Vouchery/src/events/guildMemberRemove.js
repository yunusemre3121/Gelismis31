...
const { cyronixStar, cyronixTik, cyronixRed,cyronixCikis } = require("../../../src/configs/emojis.json")
...
module.exports = async (member) => {
  ...
  if (!inviteMemberData) {
    channel.send({ content: `${cyronixCikis} \`${member.user.tag}\` sunucumuzdan ayrıldı ama kim tarafından davet edildiğini bulamadım. Sunucumuzda **${member.guild.memberCount}** Üye Kaldi `});
  } else {
    ...
    channel.send({ content:`${cyronixCikis} \`${member.user.tag}\` sunucumuzdan ayrıldı. ${inviter.tag} tarafından davet edilmişti. (**${total}** daveti) Sunucumuzda **${member.guild.memberCount}** Üye Kaldi `});
