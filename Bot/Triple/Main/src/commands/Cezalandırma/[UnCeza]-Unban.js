...
const { cyronixRed, cyronixTik, cyronixCeza, cyronixRevu, cyronixOk } = require("../../../../src/configs/emojis.json")
const ayar = require("../../../../src/configs/ayarName.json");
module.exports = {
  conf: {
    aliases: ["unban"],
    name: "unban",
    help: "unban <cyr0nix/ID>",
  ...
  run: async (client, message, args, embed) => {
    ...
    message.reply({ content:`${cyronixTik} \`(${ban.user.username.replace(/\`/g, "")} - ${ban.user.id})\` adlı üyenin banı ${message.author} tarafından kaldırıldı!`}).then((e) => setTimeout(() => { e.delete(); }, 50000));
    ...
    .addFields(
      { name: "Affedilen",  value: `<@${ban.user.id}>`, inline: true },
