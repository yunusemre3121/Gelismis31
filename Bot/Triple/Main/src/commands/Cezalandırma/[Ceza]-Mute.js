...
const { cyronixRed, cyronixTik, cyronixSaat, cyronixOk } = require("../../../../src/configs/emojis.json")
...
module.exports = {
  conf: {
    aliases: ["mute","cmute"],
    name: "mute",
    help: "mute <cyr0nix/ID> <Süre> <Sebep>",
  ...
  run: async (client,message, args, embed) => {
    ...
    if (duration) {
      ...
      if(msg) msg.delete();
      await message.channel.send({ content:`${cyronixTik} ${member.toString()} üyesi, ${message.author} tarafından, \`${reason}\` nedeniyle susturuldu! \`(Ceza ID: #${penal.id})\``}).then((e) => setTimeout(() => { e.delete(); }, 50000));
       message.react(cyronixTik)
    ...
    const penal = await client.penalize(message.guild.id, member.user.id, "CHAT-MUTE", true, message.author.id, reason, true, Date.now() + duration);
    if(msg) msg.delete();
    interaction.followUp({ content:`${cyronixTik} ${member.toString()} üyesi, ${message.author} tarafından, \`${reason}\` nedeniyle susturuldu! \`(Ceza ID: #${penal.id})\``}).then((e) => setTimeout(() => { e.delete(); }, 50000));
    if (allah.Main.dmMessages) member.send({ content:`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından, **${reason}** sebebiyle, <t:${Math.floor((Date.now() + duration) / 1000)}:R>'ya kadar susturuldunuz.`}).catch(() => {});
    ...
    await interaction.deferUpdate();
    if(msg) msg.delete();
    interaction.followUp({ content: `${cyronixTik} Susturma işlemi başarıyla iptal edildi.`, ephemeral: true });
    }
    })
    }
      },
