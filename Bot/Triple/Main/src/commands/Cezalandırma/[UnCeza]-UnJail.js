...
const { cyronixRed, cyronixTik, cyronixRevu} = require("../../../../src/configs/emojis.json")
const ayar = require("../../../../src/configs/ayarName.json");
module.exports = {
  conf: {
    aliases: ["unjail"],
    name: "unjail",
    help: "unjail <cyr0nix/ID>",
  ...
  run: async (client, message,  args, embed) => {
    ...
    const data = await penals.findOne({ userID: member.user.id, guildID: message.guild.id, $or: [{ type: "JAIL" }, { type: "TEMP-JAIL" }, { type: "UNDERWORLD" }], active: true });
    if (data) {
      data.active = false;
      await data.save();
    }
    message.react(cyronixTik)
    message.reply({ content:`${cyronixTik} ${member.toString()} üyesinin jaili ${message.author} tarafından kaldırıldı!`}).then((e) => setTimeout(() => { e.delete(); }, 50000));
    if (allah.Main.dmMessages) member.send({ content:`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından, jailiniz kaldırıldı!`}).catch(() => {});
          ...
          .addFields(
            ...
            { name: "Affeden",  value: `<@${message.author.id}>`, inline: true },
