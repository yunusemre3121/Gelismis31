...
const { cyronixRed, cyronixTik, cyronixSaat, cyronixOk, cyronixCeza, cyronixRevu } = require("../../../../src/configs/emojis.json")
...
msg.edit({ content :`${cyronixTik} ${member ? member.toString() : member.user.username} üyesi, ${message.author} tarafından, \`${reason}\` nedeniyle **kalıcı olarak** banlandı! \`(Ceza ID: #${penal.id})\``, embeds: [], components: []})

const log = new EmbedBuilder() 
.setDescription(`
${cyronixCeza} Banlanan Üye: **${member ? member.toString() : member.user.username}** \`${member.user.id}\`
${cyronixRevu} Banlayan Yetkili: ${message.author} \`${message.author.id}\`
${cyronixOk} Ban Sebebi: \`${reason}\`
`)
...
if(button.customId === "üç") {
  ...
  msg.edit({ content :`${cyronixTik} ${member ? member.toString() : member.user.username} üyesi, ${message.author} tarafından, \`Reklam\` nedeniyle banlandı! \`(Ceza ID: #${penal.id})\``, embeds: [], components: []})

  const log = embed
  .setDescription(`
${cyronixCeza} Banlanan Üye: **${member ? member.toString() : member.user.username}** \`${member.user.id}\`
${cyronixRevu} Banlayan Yetkili: ${message.author} \`${message.author.id}\`
${cyronixOk} Ban Sebebi: \`Reklam\`
  `)
  .setFooter({ text:`${moment(Date.now()).format("LLL")}    (Ceza ID: #${penal.id})` })
  message.guild.channels.cache.get(conf.banLogChannel).send({ embeds: [log]});
