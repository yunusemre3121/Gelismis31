...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    if (!message.member.permissions.has(PermissionsBitField.Flags.Administrator) && !conf.staffs.some(x => message.member.roles.cache.has(x))) 
    {
    message.react(cyronixRed)
    message.channel.send({ content:"Yeterli yetkin bulunmuyor cyr0nix ile görüş !"}).then((e) => setTimeout(() => { e.delete(); }, 5000)); 
    return }
    ...
    await penals.find({ guildID: message.guild.id, userID: member.user.id, }).sort({ date: -1 }).exec(async (err, res) => {
        ...
        if (data.length === 0) return message.channel.send({ content:`${cyronixTik} ${member.toString()} üyesinin sicili temiz!`}).then((e) => setTimeout(() => { e.delete(); }, 5000));
