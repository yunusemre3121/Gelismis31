...
module.exports = {
 ...
 const logEmbed = new EmbedBuilder()
  ...
  if (client.channels.cache.find(c => c.name === "register_log")) client.channels.cache.find(c => c.name === "register_log").send({ embeds: [logEmbed] })
    ...
    member.setNickname(`${member.user.displayName.includes(ayar.tag) ? ayar.tag : (ayar.ikinciTag ? ayar.ikinciTag : (ayar.tag || ""))} İsim | Yaş`)
    message.reply({ content:`${member} üyesi, ${message.author} tarafından, kayıtsıza atıldı! ${cyronixTik}`}).then((e) => setTimeout(() => { e.delete(); }, 5000));
