...
run: async (client, message, args, embed, prefix) => { 
   ...
   const tagModedata = await regstats.findOne({ guildID: message.guild.id })
    ...
    const row = new ActionRowBuilder()
		.addComponents( 
    new ButtonBuilder().setCustomId("MAN").setLabel("Erkek").setStyle(ButtonStyle.Primary).setEmoji("1219419927065137202"),
    new ButtonBuilder().setCustomId("WOMAN").setLabel("Kadın").setStyle(ButtonStyle.Success).setEmoji("1219419998242738206"),
...
${cyronixRed} üyesinin toplamda **${data ? `${data.names.length}` : "0"}** isim kayıtı bulundu
${data ? data.names.splice(-3).map((x, i) => `\` ${i + 1} \` \` ${x.name} \` ${x.sebep ? `(${x.sebep})` : ""} ${x.rol ? `(${x.rol})` : ""}`).join("\n") : "Daha önce kayıt olmamış."}

Üyesinin önceki isimlerine \`.isimler <@cyr0nix/ID>\` komutuyla bakarak kayıt işlemini gerçekleştirmeniz önerilir.
`)
...
if(button.customId === "MAN") {
  ...
  let ozie2 = new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId('done')
  ...
  if(msg) msg.delete();
  message.reply({ embeds: [ozie], components: [ozie2]});
...
let ozik2 = new ActionRowBuilder().addComponents(
  new ButtonBuilder()
    .setCustomId('done')
    .setEmoji('1213638247918346350')
    .setStyle(ButtonStyle.Success)
...
if(msg) msg.delete();
message.reply({ embeds: [ozik], components: [ozik2]});
...
if(msg) msg.delete();
message.reply({ content:`İşlem Başarıyla İptal Edildi ${cyronixTik}`, embeds: [], components: []});
