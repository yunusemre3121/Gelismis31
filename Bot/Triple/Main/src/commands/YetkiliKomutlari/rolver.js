...
module.exports = {
  conf: {
    ...
    name: "rolver",
    help: "rolver <cyr0nix/ID> <Role/ID>",
    category: "yetkili",
  },
  ...
  run: async (client, message, args, embed) => {
    ...
    if (!args[0]) return message.reply({ content:`${cyronixRed} Kullanımı: !r al/ver Kullanıcı Rol`})
    if (args[0] != "al") {
        if (args[0] != "ver") {
            return message.reply({ content:`${cyronixRed} Kullanımı: !r al/ver Kullanıcı Rol`})
        }
    }

    if (!args[1]) return message.reply({ content:`${cyronixRed} Bir üye etiketle ve tekrardan dene!`})
    let rMember = message.mentions.members.first() || message.guild.members.cache.get(args[1])
    if (!rMember) return message.reply({ content:`${cyronixRed} Bir üye etiketle ve tekrardan dene!`})

    if (!args[2]) return message.reply({ content:`${cyronixRed} Rolü belirtmelisin.`})
    let role = message.mentions.roles.first() || message.guild.roles.cache.get(args[2])
        if (!role) return message.reply({ content:`${cyronixRed} Belirtmiş olduğun rolü bulamadım ! Düzgün bir rol etiketle veya ID belirtip tekrar dene.`})
        if (message.member.roles.highest.rawPosition <= role.rawPosition) return message.reply({ content:`${cyronixRed} Kendi rolünden yüksek veya eşit bir rolle işlem yapamazsın.`})
