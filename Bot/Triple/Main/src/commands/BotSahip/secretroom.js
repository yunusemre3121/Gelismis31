const { AuditLogEvent, PermissionsBitField, TextInputStyle, TextInputBuilder, ModalBuilder, ChannelType, ButtonStyle, ComponentType, Discord, EmbedBuilder, StringSelectMenuBuilder, ButtonBuilder, ActionRowBuilder, PermissionFlagsBits } = require("discord.js");
const allah = require("../../../../../../config.json");
const conf = require("../../../../src/configs/sunucuayar.json")
const { cyronixRed, cyronixTik, secret1, secret2, secret3, secret4, secret5, secret6, secret7, secret8, secret9, secret10, cyronixOk } = require("../../../../src/configs/emojis.json");
...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
	...
	let embedx = new EmbedBuilder()
        .setAuthor({ name: `${guild.name} Özel Oda Sistemi`, iconURL: `${message.guild.iconURL({ dynamic: true})}`})
        .setColor("#fc03b1")
...
const Channel = Guild?.channels.cache.get(room?.ID);
    ...
    return }
        await Guild?.channels?.cache.get(room?.ID)?.permissionOverwrites.edit(allah.GuildID, { Connect: false, SendMessages: false });
    ...
    collector.on('end', i => {
       msg.delete();
        ...
        await interaction.reply({ content: `Lütfen kanalınıza girmesini istediğiniz üye(leri) etiketleyiniz. \`Örn: @cyr0nix\``, ephemeral: true })
       ...
       let yarak = await interaction.channel.send({ content: `Odaya erişimini kaldırmak istediğiniz üyeleri listeden seçiniz.`, components: [row]});
        ...
        await interaction.reply({ content: `Kanal sahipliğini aktarmak istediğiniz üyeyi etiketleyiniz. \`Örn: @cyr0nix\``, ephemeral: true })
        interaction.channel.awaitMessages({ filter, max: 1 }).then(async function (collected) {
