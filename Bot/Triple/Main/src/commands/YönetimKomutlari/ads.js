  ...
  run: async (client, message, args, embed) => {
    ...
    .then(() => {
      message.channel.awaitMessages({
        ...
        message.reply({ content :`${cyronixTik} ${member.toString()} üyesi, ${message.author} tarafından, \`${reason}\` nedeniyle jaillendi! \`(Ceza ID: #${penal.id})\``});
        ...
        }).catch(err => message.reply({ content :`${cyronixRed} 2 dakika içerisinde kanıt ekran görüntüsü atılmadığı için iptal edildi.`}));
      });
