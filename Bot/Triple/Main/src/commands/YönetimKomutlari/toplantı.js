...
const { cyronixStar, cyronixRed, cyronixTik } = require("../../../../src/configs/emojis.json");
...
embed.addFields({ name: `${cyronixStar} Son Toplantı Bilgisi`, value: `
> Toplantıya **\`${MeetingData.Joining.length}\`** yetkili katılmış ${cyronixTik}
> Toplantıya **\`${MeetingData.Leaving.length}\`** yetkili katılmamış ${cyronixRed}
...
**${guild.name}** sunucusunda şuan da toplantı durumu: **${meetingStatus ? "Aktif" : "Devre-dışı"}** ${meetingStatus ? cyronixTik : cyronixRed}`)
...
${cyronixStar} **Bitirilen Toplantı Bilgisi**
> Bu toplantıda katılması beklenen **(\`${KatilimData.Leaving.length + KatilimData.Joining.length} yetkili\`)**
> Bu toplantıda katılım sağlayan: **(\`${KatilimData.Joining.length} yetkili\`)** ${cyronixTik}
> Bu toplantıda katılım sağlamayan: **(\`${KatilimData.Leaving.length} yetkili\`)** ${cyronixRed}
> Bu toplantı  **\`${meetingTime(KatilimData.endDate - KatilimData.Date)}\`** sürmüş.`)], components: []})
                ...
                if (datax2) {
                  let enAltYetkiliRolü = message.guild.roles.cache.get(ayar.staffs[0])
