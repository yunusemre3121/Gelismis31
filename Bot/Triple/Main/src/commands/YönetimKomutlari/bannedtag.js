...
module.exports = {
    ...
    run: async (client, message, args, perm, prefix) => {
        ...
        if (!args[0]) {
            message.channel.send({ content: `\`${prefix}yasaklı-tag <ekle/kaldır/liste/kontrol>\`` }).then((e) => setTimeout(() => { e.delete(); }, 5000));
            return
        }
        ...
        if (args[0] == "ekle") {
            ...
            if (!data) {
                ...
                let üyeler = message.guild.members.cache.filter(x => {
                    return x.displayName.includes(args[1])
                })
        ...
        if (args[0] == "liste" && args[1] == "üye") {
            ...
            let üyeler = message.guild.members.cache.filter(x => {
                return x.displayName.includes(args[2])
            }).map(x => "<@" + x.id + "> - (`" + x.id + "`)")
            let üyelerk = message.guild.members.cache.filter(x => {
                return x.displayName.includes(args[2])
            }).map(x => "" + x.displayName + " - (`" + x.id + "`)")
        ...
        if (args[0] == "kontrol") {
            ...
            data.taglar.forEach(tag => {
                let membersToBan = message.guild.members.cache.filter(member => {
                    // Her üyenin takma adı ve kullanıcı adı için debug logu
                    let displayName = member.nickname || member.user.username || member.displayName;
                    return displayName.includes(tag) && !member.roles.cache.has(conf.yasaklıRole[0]);
                }).map(member => member.id); // Üye ID'lerini topla

                // Yasaklanacak üyeler için debug logu
                console.log(`Yasaklı tag ${tag} ile yasaklanacak üyeler: ${membersToBan.join(", ")}`);

                // Güncellenmesi gereken üye sayısıyla ilgili mesaj gönder
                message.channel.send({ content: `${tag} tagı bulunup <@&${conf.yasaklıRole[0]}> rolü olmayan ${membersToBan.length} kişiye rolü veriyorum.` });

                // Her üyeye rolü atamak için zamanlı bir döngü kullan, API hız limitlerini önlemek için
                for (let i = 0; i < membersToBan.length; i++) {
                    setTimeout(() => {
                        let member = message.guild.members.cache.get(membersToBan[i]);
                        if (member) {
                            // Rol atanmasını doğrulamak için debug logu
                            console.log(`Üyeye rol atanıyor: ${member.nickname || member.user.username}`);

                            // Yasaklı rolü atama, booster rolü varsa koruyarak
                            if (member.roles.cache.has(conf.boosterRolu)) {
                                member.roles.set([conf.boosterRolu, conf.yasaklıRole[0]]);
                            } else {
                                member.roles.set(conf.yasaklıRole);
                            }
                        }
                    }, (i + 1) * 1000);
                }
