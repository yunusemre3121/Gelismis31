...
module.exports = {
    conf: {
      aliases: ["etiketrol"],
      name: "etiketrol",
      help: "etiketrol <Tag> <Etiket> <@Rol/ID>",
      category: "yönetim",
    },
...
message.guild.members.cache.filter(s => s.user.discriminator === etiket || s.user.displayName.includes(tag) && !s.roles.cache.has(rol)).forEach(m => m.roles.add(rol))
