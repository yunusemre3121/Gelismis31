...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    if (!conf.staffs.some(x => message.member.roles.cache.has(x))) return message.react(cyronixRed)
...
let collector = await msg.createMessageComponentCollector({ filter, time: 60000 })
    ...
    collector.on("collect", async (button) => {
      ...
      .setAuthor({ name: member.displayName, iconURL: member.user.avatarURL({ dynamic: true })})
      .setTimestamp()
      .setDescription(`${message.author}, ${member.toString()} Adlı kullanıcı senin taglı aldırma isteğini onayladı. ${cyronixTik}`)
      ...
      .setFooter({ text: message.author.tag, iconURL: message.author.avatarURL({ dynamic: true })})
      .setTimestamp()
      .setDescription(`${message.author}, ${member} Adlı kullanıcı senin taglı aldırma isteğini onaylamadı. ${cyronixRed}`)
