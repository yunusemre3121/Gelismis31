  ...
  return message.channel.send({ content: `Bu komutu kullanabilmek için ayarlanan sahip rolüne sahip olmalısınız!`}).then((e) => setTimeout(() => { e.delete(); }, 5000));
    ...
    if (args[0] === "ekle" || args[0] === "add") {
      ...
      if (coinData && client.ranks.some(x => coinData.coin >= x.coin && !member.roles.cache.has(x.role))) {
        ...
        client.channels.cache.find(x => x.name == "yetkili_atlama").send({ embeds: [embed.setDescription(`${member.toString()} üyesine ${message.member.toString()} tarafından **${count}** adet coin eklendi ve kişiye ${roles.filter(x => roles.indexOf(x) === roles.length-1).map(x => `<@&${x.role}>`).join("\n")} rolleri verildi!`)]});
      }
      ...
      if (coinData && client.ranks.some(x => coinData.coin < x.coin && member.roles.cache.has(x.role))) {
        ...
        client.channels.cache.find(x => x.name == "yetkili_atlama").send({ embeds: [embed.setDescription(`${member.toString()} üyesinden ${message.member.toString()} tarafından **${count}** adet coin çıkarıldı ve kişiden ${roles.map(x => `<@&${x.role}>`).join(", ")} rolleri alındı!`)]});
      }
