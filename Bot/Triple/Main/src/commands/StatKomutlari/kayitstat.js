...
const { cyronixRed, cyronixTik, cyronixSonsuz } = require("../../../../src/configs/emojis.json")
...
module.exports = {
  ...
  run: async (client, message, args, embed, prefix) => { 
    ...
    return }
    if (args[0] === `${member}`) {
      let data = await regstats.findOne({ guildID: message.guild.id, userID: member.id });
      message.react(cyronixTik)
      message.reply({ embeds: [embed.setDescription(`  
${member} **Kullanıcısının kayıt verileri;**
> ${cyronixSonsuz} Toplam kayıt bilgisi: \`${data ? data.top : 0}\`
> ${cyronixSonsuz} Toplam erkek kayıt bilgisi: \`${data ? data.erkek : 0}\`
> ${cyronixSonsuz} Toplam kız kayıt bilgisi: \`${data ? data.kız : 0}\`
	`)] });
    }
    else if (args[0] === "top") {
    ...
    `)
      ...
      const data = await regstats.findOne({ guildID: message.guild.id, userID: message.author.id });
      message.react(cyronixTik)
      message.reply({ embeds: [embed.setDescription(`  
${cyronixSonsuz} Toplam kayıt bilgisi: \`${data ? data.top : 0}\`
${cyronixSonsuz} Toplam erkek kayıt bilgisi: \`${data ? data.erkek : 0}\`
