...
const {  cyronixOk, cyronixStar, cyronixSonsuz } = require("../../../../src/configs/emojis.json");
...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    let tagged;
    if (conf.tag && conf.tag.length > 0) tagged = invMember ? message.guild.members.cache.filter((m) => invMember.some((x) => x.userID === m.user.id) && m.user.displayName.includes(conf.tag)).size : 0;
    else tagged = 0;
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(`
${cyronixStar} Toplam **${total}** davet. \`(${regular} gerçek, ${bonus} bonus, ${leave} ayrılmış, ${fake} fake)\`
