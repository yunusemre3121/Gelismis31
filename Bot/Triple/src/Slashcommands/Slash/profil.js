...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("profil")
    .setDescription("İstatistik bilgilerinizi gönderir."),
  ...
  async execute(interaction, client) {
    ...
    const toplamData = await toplams.findOne({ guildID: interaction.guild.id, userID: interaction.user.id });


const inviterData = await inviterSchema.findOne({ guildID: interaction.guild.id, userID: interaction.user.id });
    const total = inviterData ? inviterData.total : 0;

        const category = async (parentsArray) => {
        const data = await voiceUserParent.find({ guildID: interaction.guild.id, userID: interaction.user.id });
...
const ctx = canvas.getContext('2d');
 ...
 let ozi = Math.floor((Math.random() * Jaylen.length));
  ...
  const yazı = [] 
  if(interaction.user.username.length > 8) {
    let yarrak = interaction.user.username.slice(0, 8)
     yazı.push(`${yarrak}...`)  
        } else {
    yazı.push(`${interaction.user.username}`)
            }
  ...
  ////////////////bitiş////////////////////////////////////////////////  
const avatar = await Canvas.loadImage(interaction.member.displayAvatarURL({ extension: 'png' }));
 ctx.save();
  ...
  const attachment = new AttachmentBuilder(canvas.toBuffer(), { name: 'cyr0nix.png'});
  interaction.reply({ content:`> **[** \`${interaction.user.tag}\` **]** kullanıcısının sunucu veri kartı!`, files: [attachment], ephemeral: true })
