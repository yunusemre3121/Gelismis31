...
module.exports = async (message) => {
    ...
    if (message.member.roles.cache.some(rol => komut.YetkiliRol.includes(rol.id)) || message.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
        if (!üye) return message.reply({ embeds: [new EmbedBuilder().setDescription(`${cyronixRed} Lütfen rol verilecek kişiyi etiketle.`)] }).then((e) => setTimeout(() => { e.delete(); }, 5000)).catch(err => {});
        if (!komut.verilecekRol.some(rol => üye.roles.cache.has(rol))) {
            üye.roles.add(komut.verilecekRol)
            message.react(cyronixTik)
            message.channel.send({ embeds: [new EmbedBuilder().setDescription(`${cyronixTik} Başarılı şekilde ${üye} kişisine ${komut.verilecekRol.map(x => `<@&${x}>`)} rolünü verdim.`)] }).then((e) => setTimeout(() => { e.delete(); }, 5000)).catch(err => {});
            channel.send({ embeds: [new EmbedBuilder().setDescription(`${message.member} yetkilisi tarafından ${üye} kişisine ${komut.verilecekRol.map(x => `<@&${x}>`)} rolü verildi.`)] })
        } else {
            üye.roles.remove(komut.verilecekRol)
            message.react(cyronixTik)
            message.channel.send({ embeds: [new EmbedBuilder().setDescription(`${cyronixTik} Başarılı şekilde ${üye} kişisinden ${komut.verilecekRol.map(x => `<@&${x}>`)} rolünü aldım.`)] }).then((e) => setTimeout(() => { e.delete(); }, 5000)).catch(err => {});
            channel.send({ embeds: [new EmbedBuilder().setDescription(`${message.member} yetkilisi tarafından ${üye} kişisinden ${komut.verilecekRol.map(x => `<@&${x}>`)} rolü alındı.`)] })
            }
    }
}; 
module.exports.conf = {
