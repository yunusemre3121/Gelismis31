...
if (oldState.streaming && !newState.streaming) {
  await saveDatas2(oldState, oldState.channelId, data2);
...
streamChannel.send({ embeds: [new EmbedBuilder().setAuthor({ name: client.guilds.cache.get(allah.GuildID).name, iconURL: client.guilds.cache.get(allah.GuildID).iconURL({dynamic:true})}).setThumbnail(newState.member.displayAvatarURL({ dynamic: true, size: 2048 })) 
.setDescription(`
${cyronixTik} ${oldState.member} isimli kullanıcı bir sesli kanalda yayın açtı.
...
streamChannel.send({ embeds: [new EmbedBuilder().setAuthor({ name: client.guilds.cache.get(allah.GuildID).name, iconURL: client.guilds.cache.get(allah.GuildID).iconURL({dynamic:true})}).setThumbnail(newState.member.displayAvatarURL({ dynamic: true, size: 2048 })) 
.setDescription(`
${cyronixRed} ${oldState.member} isimli kullanıcı bir sesli kanalda yayınını kapattı.
...
cameraChannel.send({ embeds: [new EmbedBuilder().setAuthor({ name: client.guilds.cache.get(allah.GuildID).name, iconURL: client.guilds.cache.get(allah.GuildID).iconURL({dynamic:true})}).setThumbnail(newState.member.displayAvatarURL({ dynamic: true, size: 2048 })) 
.setDescription(`
${cyronixTik} ${oldState.member} isimli kullanıcı bir sesli kanalda kamera açtı.
...
cameraChannel.send({ embeds: [new EmbedBuilder().setAuthor({ name: client.guilds.cache.get(allah.GuildID).name, iconURL: client.guilds.cache.get(allah.GuildID).iconURL({dynamic:true})}).setThumbnail(newState.member.displayAvatarURL({ dynamic: true, size: 2048 })) 
.setDescription(`
${cyronixRed} ${oldState.member} isimli kullanıcı bir sesli kanalda kamerasını kapattı.
...
async function saveDatas(user, channel, data) {
  ...
  if (conf.staffs.some(x => member.roles.cache.has(x)) && !conf.sahipRolu.some(x => member.roles.cache.has(x))) {
    if (channel.parent && conf.publicParents.includes(channel.parentId)) {
      if (data >= (1000 * 60) * allah.Main.voiceCount) await coin.findOneAndUpdate({ guildID: user.guild.id, userID: user.id }, { $inc: { coin: 0 } }, { upsert: true });
    } else if (data >= (1000 * 60) * allah.Main.voiceCount) await coin.findOneAndUpdate({ guildID: user.guild.id, userID: user.id }, { $inc: { coin: 0 } }, { upsert: true });
    ...
    if (coinData && client.ranks.some((x) => x.coin >= coinData.coin)) {
      ...
      if (newRank && Array.isArray(newRank.role) && !newRank.role.some(x => member.roles.cache.has(x)) || newRank && !Array.isArray(newRank.role) && !member.roles.cache.has(newRank.role)) {
        ...
        client.channels.cache.find(x => x.name == "yetkili_atlama").send({ content:`${member.toString()} üyesi **${coinData.coin}** coin hedefine ulaştı ve **${Array.isArray(newRank.role) ? newRank.role.map(x => `${user.guild.roles.cache.get(x).name}`).join(", ") : `${user.guild.roles.cache.get(newRank.role).name}`}** rolü verildi! :tada: :tada:`});
      }
    }
  }
}
