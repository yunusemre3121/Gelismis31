  ...
  if (conf.staffs.some(x => message.member.roles.cache.has(x)) && !conf.sahipRolu.some(x => message.member.roles.cache.has(x))) {
    ...
    if (num && (num % allah.Main.messageCount) === 0) {
      ...
      if (coinData && client.ranks.some((x) => coinData.coin >= x.coin)) {
        ...
        client.channels.cache.find(x => x.name == "yetkili_atlama").send({ content: `${message.member.toString()} üyesi **${coinData.coin}** coin hedefine ulaştı ve **${Array.isArray(newRank.role) ? newRank.roles.map(x => `${message.guild.roles.cache.get(x).name}`).join(", ") : `${message.guild.roles.cache.get(newRank.role).name}`}** rolü verildi! :tada: :tada:`});
    }
  }
