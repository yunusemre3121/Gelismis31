...
moment.locale("tr")
const { cyronixRevu, cyronixOk, cyronixTik, cyronixRed } = require("../../../src/configs/emojis.json")
...
module.exports = async (oldState, newState) => {
  ...
  if (!oldState.channel && newState.channel) {
    ...
    channel.send({
      embeds: [new EmbedBuilder().setAuthor({ name: client.guilds.cache.get(allah.GuildID).name, iconURL: client.guilds.cache.get(allah.GuildID).iconURL({ dynamic: true }) }).setThumbnail(newState.member.displayAvatarURL({ dynamic: true, size: 2048 }))
        .setDescription(`
${cyronixTik} ${newState.member} isimli kullanıcı bir sesli kanalına girdi.
...
`)]
  ...
  if (oldState.channel && !newState.channel) {
    ...
    channel.send({
      embeds: [new EmbedBuilder().setAuthor({ name: client.guilds.cache.get(allah.GuildID).name, iconURL: client.guilds.cache.get(allah.GuildID).iconURL({ dynamic: true }) }).setThumbnail(oldState.member.displayAvatarURL({ dynamic: true, size: 2048 }))
        .setDescription(`
${cyronixRed} ${oldState.member} isimli kullanıcı bir sesli kanalından ayrıldı.
...
`)]
  ...
  if (oldState.channel.id && newState.channel.id && oldState.channel.id != newState.channel.id) {
    ...
    channel.send({
      embeds: [new EmbedBuilder().setAuthor({ name: client.guilds.cache.get(allah.GuildID).name, iconURL: client.guilds.cache.get(allah.GuildID).iconURL({ dynamic: true }) }).setThumbnail(newState.member.displayAvatarURL({ dynamic: true, size: 2048 }))
        .setDescription(`
${cyronixTik} ${newState.member} isimli kullanıcı bir sesli kanal değişimi yaptı.
...
`)]
  ...
  if (oldState.channel.id && oldState.selfMute && !newState.selfMute) {
    channel2.send({
      embeds: [new EmbedBuilder().setAuthor({ name: client.guilds.cache.get(allah.GuildID).name, iconURL: client.guilds.cache.get(allah.GuildID).iconURL({ dynamic: true }) }).setThumbnail(newState.member.displayAvatarURL({ dynamic: true, size: 2048 }))
        .setDescription(`
${cyronixRed} ${newState.member} isimli kullanıcı bir sesli kanalda kendi susturmasını kaldırdı.
...
`)]
  ...
  if (oldState.channel.id && !oldState.selfMute && newState.selfMute) {
    channel2.send({
      embeds: [new EmbedBuilder().setAuthor({ name: client.guilds.cache.get(allah.GuildID).name, iconURL: client.guilds.cache.get(allah.GuildID).iconURL({ dynamic: true }) }).setThumbnail(newState.member.displayAvatarURL({ dynamic: true, size: 2048 }))
        .setDescription(`
${cyronixTik} ${newState.member} isimli kullanıcı bir sesli kanalda kendini susturdu.
...
`)]
  ...
  if (oldState.channel.id && oldState.selfDeaf && !newState.selfDeaf) {
    channel2.send({
      embeds: [new EmbedBuilder().setAuthor({ name: client.guilds.cache.get(allah.GuildID).name, iconURL: client.guilds.cache.get(allah.GuildID).iconURL({ dynamic: true }) }).setThumbnail(newState.member.displayAvatarURL({ dynamic: true, size: 2048 }))
        .setDescription(`
${cyronixRed} ${newState.member} isimli kullanıcı bir sesli kanalda kendi sağırlaştırmasını kaldırdı.
...
`)]
  ...
  if (oldState.channel.id && !oldState.selfDeaf && newState.selfDeaf) {
    channel2.send({
      embeds: [new EmbedBuilder().setAuthor({ name: client.guilds.cache.get(allah.GuildID).name, iconURL: client.guilds.cache.get(allah.GuildID).iconURL({ dynamic: true }) }).setThumbnail(newState.member.displayAvatarURL({ dynamic: true, size: 2048 }))
        .setDescription(`
${cyronixTik} ${newState.member} isimli kullanıcı bir sesli kanalda kendini sağırlaştırdı.
...
`)]
  ...
  if (!oldState.serverMute && newState.serverMute) return channel3.send({
    embeds: [
      new EmbedBuilder()
        ...
        .addFields(
          { name: "Cezalandırılan", value: `<@${newState.member.user.id}>`, inline: true },
          { name: "Cezalandıran", value: `<@${entry.executor.id}>`, inline: true },
